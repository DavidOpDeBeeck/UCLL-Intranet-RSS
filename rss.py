...
for page_element in enumerate(page_items_html[:-2]):  # Remove 'volgende' and 'last' from the page items
    ...
    for messages_h2_index, message_h2_element in enumerate(messages_h2):
        if message_h2_element.a is not None:
            ...
            if database.get_item(message_url) is None:
                ...
                title = message_h2_element.a.text
                description = ''
                author = message_author
                categories = []

                if args.initialise:
                    pub_date = time.mktime(datetime.strptime(message_publish_date, '%d-%m-%Y').timetuple())
                else:
                    pub_date = time.mktime(datetime.now().timetuple())
                ...
                if message_categories and message_categories.find_all('a'):  # Only add categories if found
                    for message_category in message_categories.find_all('a'):
                        categories.append((category_prefix_url + message_category['href'], message_category.text))

                database.save_item(message_url, title, description, pub_date, author, categories)
